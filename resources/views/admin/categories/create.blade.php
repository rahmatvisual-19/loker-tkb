...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sm:p-8 max-w-2xl mx-auto mb-10">
    ...
    <form action="{{ route('categories.store') }}" method="POST" class="space-y-5">
        ...
        <div>
            ...
            <input type="text" name="name" class="w-full border border-gray-300 focus:border-green-500 focus:ring-1 focus:ring-green-500 rounded-lg p-2.5 transition-colors" placeholder="Misal: IT & Software" required>
        </div>

        <div class="pt-4 border-t border-gray-100">
            <button type="submit" class="bg-green-600 hover:bg-green-700 transition-colors text-white font-medium px-6 py-2.5 rounded-lg shadow-sm">Simpan Kategori</button>
        </div>
    </form>
</div>
@endsection